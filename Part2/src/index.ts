...
const encrypt = async (
  ...
  const data= plaintext.map((e:bigint,i:number)=>{
...
const decrypt = async (
  ...
  const mimc7 = await buildMimc7();
  // const data= mimc7.multiHash([sharedKey,BigInt(ciphertext.iv)+BigInt()])
  const plaintext:Plaintext = ciphertext.data.map((e:bigint,i:number)=>{
